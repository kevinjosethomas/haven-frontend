...
export const TargetCard = props => {
  ...
  return (
    <div className="TargetCard flex flex-col items-center justify-between w-56 lg:w-card-target-lg 2xl:w-card-target-2xl 3xl:w-card-target h-80 lg:h-96 2xl:h-card-target-2xl 3xl:h-card-target
    cursor-default rounded transform transition duration-500 hover:scale-103">
      ...
      <div className="flex flex-col items-start justify-center px-5 lg:px-10 w-full h-3/4 bg-light-300 bg-opacity-20 space-y-5 rounded-b">
        { props.content.map(item => (
          <div key={item.text} className="font-inter font-medium text-xs lg:text-md xl:text-xl 3xl:text-2xl text-gray-500">
