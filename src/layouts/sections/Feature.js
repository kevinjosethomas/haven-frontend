...
export const FeatureSection = props => {
  ...
  return (
    <div className={`flex flex-col md:flex-row items-center justify-center py-16 md:py-0 px-5 md:px-0 w-full h-auto md:h-section-lg 3xl:h-section ${props.background}`}>
      <div className="flex flex-col items-center md:items-start justify-center order-2 md:order-none mr-4 mt-4 md:mt-0 lg:mr-2 xl:mr-10 max-w-sm 3xl:max-w-5xl">
        ...
        <p className="mt-2 lg:max-w-3xl font-inter font-medium text-xl md:text-2xl 3xl:text-3xl 3xl:text-4xl text-gray-700 text-center md:text-left">{ props.description }</p>
      </div>
      <img 
        draggable="false"
        className="order-1 md:order-none ml-4 mb-4 md:mb-0 lg:ml-2 xl:ml-10 w-64 lg:w-80 3xl:w-auto"
